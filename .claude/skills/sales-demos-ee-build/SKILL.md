...
```bash
./utilities/run-ansible.sh playbooks/validate.yml --check -i inventory --limit sandbox -e target_env=sandbox \
  --vault-id sales.demos@~/secrets/.vault_pass_sales_demos   # check mode first; --check is required (#173)
./utilities/run-ansible.sh playbooks/config.yml   -i inventory --limit sandbox -e target_env=sandbox \
