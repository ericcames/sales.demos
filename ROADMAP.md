...
| 5 | [`/sales-demos-facts`](https://github.com/ericcames/sales.demos/blob/main/.claude/skills/sales-demos-facts/SKILL.md) | `playbooks/check_{linux,windows}_vm.yml` | Day 2 begins. Fact gathering folded into step 5 of both Day 1 chains and offered off-chain as `Linux Day 2 - Gather Facts` / `Windows Day 2 - Gather Facts`. The full set is in AAP's database (`use_fact_cache` has been on since [#47](https://github.com/ericcames/sales.demos/issues/47) and nothing said so), a curated summary prints in the job log, and the same summary is published at `<web_url>/facts.html`. | **In test** ([#647](https://github.com/ericcames/sales.demos/issues/647)) |
