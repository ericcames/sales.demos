...
#
# The prompts use read -s, so the values never reach the terminal, the shell
# history, or a Claude Code transcript. They are handed to python through the
...
# ── Prompt ─────────────────────────────────────────────────────────────
echo "Setting RHDP passwords for '$ENV_NAME'. Input is hidden; Enter keeps the current value."
read -rsp "  aap_password: " NEW_AAP_PASSWORD; echo
read -rsp "  kubeadmin_password: " NEW_KUBEADMIN_PASSWORD; echo
