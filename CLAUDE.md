...
generated by `utilities/make-aap-mcp.sh` (#515). Both patterns name paths in the
tracked file, never credentials. The AAP bridge uses `npx supergateway` to
convert stdio to the remote Streamable HTTP endpoint.

The environment is in each server's *name* — `openshift-sandbox`,
`openshift-demo`, `openshift-edge`, `aap-sandbox`, `aap-demo` — because one
